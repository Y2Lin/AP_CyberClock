...
- 固件 v10.8（精简模式 v4.7——去除重影，干净主字）：精简模式时间显示的两色差重影层（v10.5 引入的双色错位副本）全部移除。精简模式只留干净主色大字、秒进度下划线、日期行与低电红点——无任何文字装饰效果。完整表盘页重影保持不变。应用段体积进一步减小。

- 固件 v10.7（精简模式 v4.6——去除光晕，回归重影）：v10.6 引入的 16 层贴字形文字光晕在真机 OLED 上观感发虚、不够利落。回退为与完整表盘页一致的双色差重影效果——两层错位文本副本（残影 1：左偏 4px / 30% 透明次色，残影 2：右偏 6px 下移 3px / 15% 透明次色）叠在主字下方。移除全部余弦缓动呼吸动效与 16 层光晕；主字未同步半透明闪烁与重影层保留。墨水屏模式仍隐藏两层残影。应用段体积相应减小（减少 16 个 label 对象）。

- 固件 v10.6（精简模式 v4.5——大字、粗线、顺滑呼吸）：按定稿设计图 v4.5，精简模式 HH:MM 从 48px 放大到 **64px**（montserrat 内置最大 48px，用 `lv_font_conv` 生成了数字子集 `lv_font_montserrat_64_digits.c`——12 个字形：0-9 + 冒号 + 空格，bpp4，flash 约 8 KB）；秒进度下划线从 112×2 加粗到 **150×3 px**（仍为大字宽的 4/5，3px 粗），距主字底约 14px；文字光晕从 12 层（1-3px）扩展到 **16 层（2-5px）**，分四档亮度环（对角 100 / 内圈十字 80 / 中圈十字 55 / 外圈十字 30 透明度），近似 CSS 双层 text-shadow blur 效果（贴字 + 铺开）。呼吸动效不再是 1Hz 二态硬切，改为 **3 秒余弦缓动曲线**（30 步查找表，每 100ms tick 推进），且只作用于光晕层——主字保持稳定不闪。日期行下移至 y=210。五套主题与墨水屏模式全部适配。应用段 1,050,480 → 1,058,528 字节（+8,048，主要是 64px 字体）；合并镜像 1,124,064 字节。
