...
- 出厂默认 80%（v10.1），用户调整后持久化到 NVS，重启自动恢复
- **空闲自动降亮**（v10.2）：90 秒无按键背光自动降到 20%，按任意键立即恢复设定亮度（设定值本身不变）

### 功耗（v10.2）
- 默认主频 160→80MHz，并开启 DFS 动态调频：忙时 80MHz、空闲降到 40MHz
- 表盘页之外自动放宽刷新节拍（100ms→250ms），减少空转唤醒
- 不使用 light sleep：USB 控制台与 USB 对时需保持在线
- 蓝牙只属于时间同步页（见下），表盘待机零射频功耗

### 其他
- 时间与时区持久化到 NVS；运行期间每 5 分钟回写一次，断电后恢复值的偏差不超过 5 分钟（v10.1）
- 设备没有 RTC 后备电池，**重启后一律回到未对时状态**，等待 BLE / USB 重新校准（v10.1 起对全部复位源生效）
