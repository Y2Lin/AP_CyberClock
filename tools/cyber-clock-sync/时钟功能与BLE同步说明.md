...
```
开机 → [TIME SYNC 时间同步页] --按 OK 手动进入--> [表盘页]
        （蓝牙仅此页开启，                    OK 长按 ↓
         同步后停留本页）                          [菜单页]
...
### 视觉效果（v10 B1++ 定稿表盘）
- **5 套主题**可切换：蓝紫霓虹（默认）/ 传统墨水屏（纸白底墨黑字，第二位）/ 青绿单色 / 橙红暖色 / 矩阵绿，四页面统一换色
- **HUD 版式**：内嵌外框 + 上下刻度尺 + 四角双线角标；顶栏型号 / 十六进制流 / 电量%；12 根频谱条；10 段分段电池 + 底部十六进制流
- **双色差残影**：大号时间文字青/品红双色错位重影，霓虹发光感
- **故障爆发特效**（非墨水屏主题）：约 7 秒一次——色差残影分离加剧、主字微移轻闪、时间区错位切片条；另有每 3 秒换位的故障色条
- **像素心跳**：终端行心形图标按秒 lub-dub 双跳，与时间主行垂直中心对齐
- **秒数进度柱**：秒数旁垂直发光柱，随秒自下而上填充
- **墨水屏主题静态化**：纸纹质感、无残影无发光无任何故障特效，静默显示
- **未同步闪烁**：时间未校准时主文字每秒明暗闪烁、同步图标同步闪烁提醒（v7 起）

### 时间显示
- 双色差残影大字号 `HH:MM`（48px 字体）+ 右侧 `SS` 秒数与垂直进度柱
- 日期 `YYYY-MM-DD` + 星期 + 终端行时区（`> UTC+8_` 闪烁光标）+ 像素心跳 + 运行时长
- 10 段分段电池 + 电量百分比 + 电压，顶栏另有电量%读数
- 2 种显示模式：完整 / 精简（隐藏状态行与电压，且精简模式禁用故障特效）
...
- 支持时区偏移设置
- 连接后每 5 秒通知当前时间状态
- 同步成功后停留同步页实时显示状态（连接保持）；按 OK 进表盘时蓝牙关闭（v9：不自动跳转）
...
| 页面 | 按键 | 动作 | 功能 |
|---|---|---|---|
| **时间同步页**（开机首屏） | OK | 短按 | 进入表盘（同步完成后设备停留本页，不自动跳转） |
| | OK | 长按 | 进入菜单 |
...
- 对时完成后**直接关闭网页不会重启设备**（v9.1 起页面保持 DTR/RTS 常低；旧版页面关闭时设备会重启一次，见故障排除）
...
| `main/cyber_clock.c` | 时钟 UI + 四页面状态机（同步页/表盘/菜单/亮度，v8）+ v10 B1++ 定稿表盘（5 主题/故障特效/像素心跳）+ 按键交互 + 亮度持久化 |
...
| `sdkconfig.defaults` | 启用 montserrat 12/14/16/20/24/48 六档字体，LVGL 内存池 24KB→32KB |
...
- 配合 v9 动线：开机首屏即时间同步页并自动开蓝牙广播（`BT: ADVERTISING`），插 USB 线或连蓝牙对时一次即恢复正常（文字停止闪烁、变 `SYNCED`；设备停留同步页，按 OK 进表盘）

### BLE 与 Wi-Fi 共存
当前固件中 BLE 和 Wi-Fi 堆栈**不能同时运行**（进入页面时初始化，退出时释放）。时钟页面只使用 BLE，不影响其他 demo 页面的 Wi-Fi 功能。

### 内存占用
- montserrat 六档字体（12/14/16/20/24/48px）按需启用，仅占用 Flash 空间（运行时按需渲染，不占 RAM）
- LVGL 内存池从 24KB 增加到 32KB，容纳 v10 表盘更多 UI 对象和动效
...
| **USB 对时完成后关闭网页/终端，设备重启了** | ESP32-C3 内置的 USB 下载复位通路会把主机**关闭串口时的 DTR/RTS 电平变化**当作复位脉冲（与 esptool 刷机后硬复位同一条通路），是芯片特性而非固件故障。**v9.1 起**网页与 `usb-sync.py` 在打开串口后即保持 DTR/RTS 常低，关闭不再触发重启；用旧版页面或第三方串口终端仍可能触发——重启后时间回到 NVS 恢复值并显示未同步（v7 诚实状态），重新对时一次即可 |
...
| **设备离开同步页后蓝牙断开** | v9 动线的正常行为：蓝牙只属于时间同步页，按 OK 进表盘（或长按进菜单）即断开并关闭蓝牙。需要再同步：设备长按 OK → 菜单选 3 TIME SYNC，蓝牙自动重开。同步完成后设备会停留在同步页（不自动跳转） |
