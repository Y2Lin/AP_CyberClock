...
开机 → [TIME SYNC 时间同步页] --同步成功 5 秒后自动 / OK 跳过--> [表盘页]
         （蓝牙仅此页开启）                              |
                                              OK 长按 ↓
                                                    [菜单页]
                                          1 进入表盘 | 2 亮度调整 | 3 时间同步
```

### 视觉效果
- **赛博朋克配色**：4 套主题可切换（蓝紫霓虹 / 青绿单色 / 橙红暖色 / 矩阵绿），四页面统一换色
- **HUD 装饰**：表盘四角 L 形角标 + 左右侧边刻度线
- **未同步闪烁**：时间未校准时主文字每秒明暗闪烁提醒（v7）

### 时间显示
- 大字号 `HH:MM`（48px 字体）+ 右侧小号 `SS` 秒数
- 日期 `YYYY-MM-DD` + 星期 + 时区 + 运行时长 + 电池电压/电量
- 2 种显示模式：完整 / 精简（隐藏状态行与电压）

### BLE 时间同步（仅时间同步页）
- 进入时间同步页（开机首屏）即开始 BLE 广播，名称 `CyberClock`
- 自定义 GATT 服务，手机 / Mac 可写入 Unix 时间戳
- 支持时区偏移设置
- 连接后每 5 秒通知当前时间状态
- 同步成功 5 秒后自动进入表盘并关闭蓝牙（省电）

### USB 时间同步（v5 新增，免蓝牙）
- 用**刷固件的同一条 USB 线**直接对时，走 ESP32-C3 USB Serial/JTAG 串口
- **不依赖所在页面**：同步页、表盘页、菜单页都能对时
- 浏览器直连（Chrome/Edge Web Serial）或命令行脚本（`usb-sync.py`）均可
- 与 BLE 方式互不干扰，Mac 蓝牙搜不到设备时的最省事替代

### 亮度调整（v8 新增）
- 亮度页 UP/DOWN 以 10% 步进调整 10~100%
- 设置持久化到 NVS，重启自动恢复

### 其他
- 时间持久化到 NVS，重启后恢复粗略时间
- 离开时间同步页即关闭蓝牙射频（表盘待机零蓝牙功耗）
...
> USB 对时不受页面限制：任何页面插线执行 `T <unix> <tz>` 命令即可校时；
> 蓝牙广播只在时间同步页存在，表盘页待机无蓝牙功耗。
...
> 不需要进入任何特定页面（同步页 / 表盘 / 菜单均可），不依赖任何蓝牙能力。
...
1. 设备处于 **TIME SYNC 时间同步页**（v8 开机首屏即是；若在表盘页：长按 OK → 菜单选 3 TIME SYNC），设备开始广播 `CyberClock`
...
1. 设备处于 **TIME SYNC 时间同步页**（开机首屏即是；表盘页长按 OK → 3 TIME SYNC。BLE 仅在该页面广播）
...
| `main/cyber_clock.c` | 时钟 UI + 四页面状态机（同步页/表盘/菜单/亮度，v8）+ 按键交互 + 亮度持久化 |

### 修改文件
| 文件 | 修改内容 |
|---|---|
| `main/main.c` | 开机直进 CyberClock 应用、按键全量转发（v8）；DEMOS 数组新增 CyberClock 入口；启动 USB 对时服务（v5） |
...
3. 状态栏用 `SYNCED` / `NOT SYNCED` 标记时间可信度
4. 建议每次使用前通过 BLE 重新同步

### 重启后的诚实状态（v7 起）
断电/深度睡眠重启后，恢复的时间只是"上次同步时刻的旧值"，**不可信**。v7 起设备如实反映：
- 开机自动检测重启（上次同步时刻的记录"在未来"），**清除已同步标志**——屏幕显示 `NOT SYNCED`、蓝牙通知与 USB `Q` 查询均返回 `synced:false`
- **时钟主文字闪烁**（每秒明暗一次），一眼可见"时间未校准"
- 配合 v8 动线：开机首屏即时间同步页并自动开蓝牙广播（`BT: ADVERTISING`），插 USB 线或连蓝牙对时一次即恢复正常（文字停止闪烁、变 `SYNCED`，5 秒后自动进表盘）
...
| **同步成功后蓝牙自动断开** | v8 动线的正常行为：同步页收到时间（蓝牙或 USB）约 5 秒后设备自动进入表盘、断开并关闭蓝牙。需要再同步：设备长按 OK → 菜单选 3 TIME SYNC，蓝牙自动重开 |
| **手机/Mac 扫不到设备，但以前能扫到** | ① 设备必须处于 TIME SYNC 同步页（表盘/菜单/亮度页蓝牙已关——长按 OK → 3 TIME SYNC 进入）；② 安卓手机先断开连接（被占用时设备不广播）；③ 重启手机蓝牙清缓存 |
| **Mac 网页搜不到设备** | ① 设备同步页须显示 `BT: ADVERTISING`（表盘页无蓝牙，长按 OK → 3 TIME SYNC）；② 安卓手机先断开连接（被占用时设备不广播）；③ 网页保持"仅显示名称过滤"不勾选，从全部设备列表选；④ macOS 缓存旧广播名时重启 Mac 蓝牙；⑤ Chrome 蓝牙权限必须允许。详见"Mac 同步步骤"章节；或直接改用 USB 线同步 |
| 手机扫描不到设备 | ① 设备必须处于 **TIME SYNC 时间同步页**（v8 开机首屏即是；蓝牙仅在该页广播，离开即关）；② 同步页屏幕应显示 `BT: ADVERTISING`；③ 手机蓝牙开关关闭再打开、清缓存后重新扫描；④ 靠近设备（<5m） |
| **扫描到的名字是 "CyberClock" 还是 FoloToy 名？** | 本固件广播名固定为 `CyberClock`。若看到的是其他名字（如原厂固件的设备名），说明刷的不是本固件或未处于时间同步页 |
